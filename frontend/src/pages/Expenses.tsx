...
export default function Expenses() {
  ...
  const tabCls = (active: boolean) =>
    `flex items-center gap-2 px-5 py-2.5 text-sm font-medium rounded-t-xl transition-colors cursor-pointer ${
      active
        ? isDark
          ? "bg-[#111A15] text-[#E4E6DE] border-b-2 border-emerald-500"
          : "bg-white text-slate-900 border-b-2 border-emerald-600"
        : isDark
        ? "text-[#6B7C6B] hover:text-[#B0B8A8]"
    ...
    <div className={`min-h-screen p-6 ${isDark ? "bg-[#090D0B]" : "bg-slate-50"}`}>
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className={`text-xl font-bold ${isDark ? "text-[#E4E6DE]" : "text-slate-900"}`}>{t("expenses.title")}</h1>
        ...
        {canMutate && (
          <button
            ...
            className={`inline-flex items-center gap-2 px-4 py-2 rounded-lg text-white text-sm font-medium transition-all ${isDark ? "bg-gradient-to-r from-[#22C55E] to-[#16A34A] shadow-lg shadow-emerald-500/20" : "bg-emerald-600 hover:bg-emerald-500"}`}
          >
          ...
          {fuelTotalPages > 1 && (
            <div className="flex items-center justify-between mt-4">
              ...
              <div className="flex items-center gap-1">
                <button disabled={fuelPage <= 1} onClick={() => setFuelPage((p) => p - 1)} className={`p-1.5 rounded-lg transition-colors disabled:opacity-30 ${isDark ? "hover:bg-[#1E2B22] text-[#6B7C6B]" : "hover:bg-slate-100 text-slate-500"}`}>
                  <ChevronLeft className="w-4 h-4" />
                </button>
                <button disabled={fuelPage >= fuelTotalPages} onClick={() => setFuelPage((p) => p + 1)} className={`p-1.5 rounded-lg transition-colors disabled:opacity-30 ${isDark ? "hover:bg-[#1E2B22] text-[#6B7C6B]" : "hover:bg-slate-100 text-slate-500"}`}>
        ...
        <>
          ...
          {expTotalPages > 1 && (
            <div className="flex items-center justify-between mt-4">
              ...
              <div className="flex items-center gap-1">
                <button disabled={expPage <= 1} onClick={() => setExpPage((p) => p - 1)} className={`p-1.5 rounded-lg transition-colors disabled:opacity-30 ${isDark ? "hover:bg-[#1E2B22] text-[#6B7C6B]" : "hover:bg-slate-100 text-slate-500"}`}>
                  <ChevronLeft className="w-4 h-4" />
                </button>
                <button disabled={expPage >= expTotalPages} onClick={() => setExpPage((p) => p + 1)} className={`p-1.5 rounded-lg transition-colors disabled:opacity-30 ${isDark ? "hover:bg-[#1E2B22] text-[#6B7C6B]" : "hover:bg-slate-100 text-slate-500"}`}>
                  <ChevronRight className="w-4 h-4" />
                </button>
              </div>
            </div>
