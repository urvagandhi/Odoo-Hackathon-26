...
export default function SafetyOfficerDashboard() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      hrApi.getExpiringLicenses(),
      fleetApi.listVehicles({ status: "IN_SHOP", limit: 10 }),
    ])
      .then(([kpiData, perfData, expiringRes, vehiclesRes]) => {
        setKpi(kpiData);
        setDriverPerformance(perfData as DriverPerformanceData[]);
        // getExpiringLicenses returns Driver[] directly (already unwrapped)
        const expiring = (Array.isArray(expiringRes) ? expiringRes : []) as unknown as ExpiringDriver[];
        setExpiringLicenses(expiring);
        // listVehicles returns PaginatedResponse<Vehicle> directly (already unwrapped)
        const vehicles = (vehiclesRes.data ?? []) as unknown as InShopVehicle[];
    ...
    <motion.div variants={stagger} initial="hidden" animate="visible" className="space-y-6">
      ...
      <motion.div variants={fadeIn} className="grid grid-cols-3 gap-5">
        ...
        <div className={`group ${cardClass} p-5`}>
          ...
          <h3 className={`text-sm font-semibold mb-4 relative z-10 ${isDark ? "text-slate-100" : "text-slate-900"}`}>Fleet safety score</h3>
          <div className="flex items-end justify-between mb-4 relative z-10">
            <div>
              <p className={`text-xs mb-1 ${isDark ? "text-slate-400" : "text-slate-500"}`}>Feb 2026</p>
              <div className="flex items-baseline gap-2">
                <span className={`text-3xl font-extrabold tabular-nums ${isDark ? "text-slate-100" : "text-slate-900"}`}>{avgSafetyScore}</span>
                <span className={`text-sm ${isDark ? "text-slate-500" : "text-slate-400"}`}>/ 100</span>
                <span className={`text-xs font-semibold px-1.5 py-0.5 rounded-full flex items-center gap-0.5 ${isDark ? "bg-emerald-500/10 text-emerald-400 border border-emerald-500/20" : "bg-emerald-50 text-emerald-600"}`}>
        ...
        <div className={`${cardClass} p-5`}>
          <h3 className={`text-sm font-semibold mb-4 ${isDark ? "text-slate-100" : "text-slate-900"}`}>Active Alerts</h3>
          <div className="space-y-3">
            <div className={`flex items-center justify-between p-3 rounded-xl ${isDark ? "bg-amber-500/10" : "bg-gradient-to-r from-amber-50 to-amber-100/50"}`}>
              <div className="flex items-center gap-2">
                <AlertTriangle className="w-4 h-4 text-amber-500" />
                <span className={`text-sm ${isDark ? "text-amber-400" : "text-amber-700"}`}>Expiring Licenses</span>
              </div>
              <span className={`font-bold ${isDark ? "text-amber-400" : "text-amber-700"}`}>{kpi?.alerts.expiringLicenses ?? 0}</span>
            </div>
            <div className={`flex items-center justify-between p-3 rounded-xl ${isDark ? "bg-red-500/10" : "bg-gradient-to-r from-red-50 to-red-100/50"}`}>
              <div className="flex items-center gap-2">
            ...
            <div className={`flex items-center justify-between p-3 rounded-xl ${isDark ? "bg-slate-800" : "bg-gradient-to-r from-slate-50 to-slate-100/80"}`}>
              <div className="flex items-center gap-2">
                <Shield className="w-4 h-4 text-slate-500" />
                <span className={`text-sm ${isDark ? "text-slate-300" : "text-slate-700"}`}>Suspended Drivers</span>
              </div>
              <span className={`font-bold ${isDark ? "text-slate-300" : "text-slate-700"}`}>{kpi?.alerts.suspendedDrivers ?? 0}</span>
            </div>
          </div>
        </div>
        ...
        <div className={`group ${cardClass} p-5`}>
          ...
          <div className="flex items-center justify-between mb-4 relative z-10">
            <h3 className={`text-sm font-semibold ${isDark ? "text-slate-100" : "text-slate-900"}`}>Compliance rate</h3>
            <span className={`text-[11px] font-medium px-2 py-0.5 rounded-full ${isDark ? "bg-slate-800 text-slate-400" : "bg-slate-100 text-slate-500"}`}>Live</span>
          </div>
          <div className="flex items-center gap-3 mb-3 relative z-10">
            <span className={`text-xs ${isDark ? "text-slate-400" : "text-slate-500"}`}>Target 95%</span>
      ...
      <motion.div variants={fadeIn} className="grid grid-cols-3 gap-5">
        ...
        <div className={`${cardClass} p-5 flex flex-col`}>
          <div className="flex items-center gap-3 mb-4">
            <div className={`w-10 h-10 rounded-full flex items-center justify-center ${isDark ? "bg-amber-500/10" : "bg-gradient-to-br from-amber-50 to-amber-100/50"}`}>
              <AlertTriangle className="w-5 h-5 text-amber-500" />
            </div>
            <div>
              <h4 className={`text-base font-bold ${isDark ? "text-slate-100" : "text-slate-900"}`}>License Alerts</h4>
              <p className={`text-xs ${isDark ? "text-slate-500" : "text-slate-400"}`}>{expiringLicenses.length} expiring within 30 days</p>
            </div>
          </div>
          ...
          {expiringLicenses.length === 0 ? (
            <div className="flex-1 flex items-center justify-center">
              <div className="text-center">
                ...
                <p className={`text-sm ${isDark ? "text-slate-500" : "text-slate-400"}`}>No expiring licenses</p>
              </div>
            </div>
          ) : (
            <div className="space-y-3 flex-1">
              {expiringLicenses.slice(0, 3).map((d) => (
                <div key={d.id} className={`flex items-center gap-3 p-3 rounded-xl border ${isDark ? "bg-amber-500/5 border-amber-500/10" : "bg-gradient-to-r from-amber-50/60 to-amber-100/40 border-amber-100"}`}>
                  <div className="w-9 h-9 rounded-full bg-violet-600 flex items-center justify-center text-white text-xs font-bold shrink-0 shadow-[0_0_10px_rgba(124,58,237,0.3)]">
                    {d.fullName.split(" ").map((n) => n[0]).join("").slice(0, 2).toUpperCase()}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className={`text-sm font-bold truncate ${isDark ? "text-slate-200" : "text-slate-900"}`}>{d.fullName}</p>
        ...
        <div className={`${cardClass} p-5`}>
          <div className="flex items-center justify-between mb-4">
            <h3 className={`text-base font-bold ${isDark ? "text-slate-100" : "text-slate-900"}`}>Top Drivers</h3>
            <span className={`px-2.5 py-1 text-[11px] font-bold rounded-full ${isDark ? "bg-emerald-500/10 text-emerald-400 border border-emerald-500/20" : "bg-gradient-to-r from-emerald-50 to-emerald-100/50 text-emerald-600"}`}>
              By safety score
            </span>
          </div>

          {topDrivers.length === 0 ? (
            <div className={`py-8 text-center text-sm ${isDark ? "text-slate-500" : "text-slate-400"}`}>No driver data yet.</div>
          ) : (
            <div className="space-y-3">
              {topDrivers.map((d, i) => (
                <div key={d.driverId} className={`flex items-center gap-3 p-3 rounded-xl transition-colors ${isDark ? "bg-slate-800/50 hover:bg-violet-900/20" : "bg-slate-50 hover:bg-gradient-to-r hover:from-violet-50 hover:to-violet-100/40"}`}>
                  <div className="relative">
                    <div className="w-10 h-10 rounded-full bg-violet-600 flex items-center justify-center text-white text-xs font-bold shadow-[0_0_10px_rgba(124,58,237,0.3)]">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className={`text-sm font-bold truncate ${isDark ? "text-slate-200" : "text-slate-900"}`}>{d.driverName}</p>
                    <div className="flex items-center gap-2">
                       <span className={`w-1.5 h-1.5 rounded-full ${STATUS_DOT[d.status] ?? (isDark ? "bg-slate-600" : "bg-slate-400")}`} />
                      <p className={`text-[11px] ${isDark ? "text-slate-500" : "text-slate-400"}`}>{d.tripsCompleted} trips</p>
                    </div>
                  </div>
        ...
        <div className={`${cardClass} p-5`}>
          <h3 className={`text-base font-bold mb-4 ${isDark ? "text-slate-100" : "text-slate-900"}`}>Driver Status</h3>
            ...
            <div className="relative w-24 h-24 shrink-0">
              <svg className="w-24 h-24 -rotate-90" viewBox="0 0 100 100">
                {(() => {
                  ...
                  let offset = 0;
                  return donutSegments.map(({ key, count }) => {
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                <span className={`text-xl font-extrabold ${isDark ? "text-slate-100" : "text-slate-900"}`}>{totalDrivers}</span>
                <span className={`text-[10px] ${isDark ? "text-slate-500" : "text-slate-400"}`}>total</span>
              </div>
            </div>
              ...
              {donutSegments.map((s) => (
                <div key={s.key} className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`text-xs ${isDark ? "text-slate-300" : "text-slate-600"}`}>{s.label}</span>
                  </div>
                  <span className={`text-xs font-bold ${isDark ? "text-slate-100" : "text-slate-900"}`}>{s.count}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className={`p-3 rounded-xl text-center border ${isDark ? "bg-violet-500/10 border-violet-500/20" : "bg-gradient-to-br from-violet-50 to-violet-100/30 border-violet-100"}`}>
              <p className={`text-lg font-extrabold ${isDark ? "text-violet-400" : "text-violet-700"}`}>{kpi?.drivers.expiringLicenses ?? 0}</p>
              <p className={`text-[10px] ${isDark ? "text-violet-400/70" : "text-violet-500"}`}>Expiring Licenses</p>
            </div>
            <div className={`p-3 rounded-xl text-center border ${isDark ? "bg-amber-500/10 border-amber-500/20" : "bg-gradient-to-br from-amber-50 to-amber-100/30 border-amber-100"}`}>
              <p className={`text-lg font-extrabold ${isDark ? "text-amber-400" : "text-amber-700"}`}>{kpi?.fleet.inShop ?? 0}</p>
              <p className={`text-[10px] ${isDark ? "text-amber-400/70" : "text-amber-500"}`}>Vehicles In Shop</p>
            </div>
          </div>
        </div>
      </motion.div>
      ...
      <motion.div variants={fadeIn} className="grid grid-cols-3 gap-5">
        ...
        <div className={`col-span-2 ${cardClass} p-0 overflow-hidden`}>
          <div className={`px-5 py-4 border-b flex items-center justify-between ${isDark ? "border-slate-800" : "border-slate-100"}`}>
            <div className="flex items-center gap-2">
              ...
              <h3 className={`text-base font-bold ${isDark ? "text-slate-100" : "text-slate-900"}`}>Vehicles In Shop</h3>
            </div>
            <span className={`text-xs font-semibold px-2.5 py-1 rounded-full ${isDark ? "bg-slate-800 text-slate-400" : "bg-slate-100 text-slate-400"}`}>
          ...
          {inShopVehicles.length === 0 ? (
            <div className="p-8 text-center">
              <CheckCircle2 className="w-8 h-8 text-emerald-400 mx-auto mb-2" />
              <p className={`text-sm ${isDark ? "text-slate-500" : "text-slate-400"}`}>All vehicles are operational</p>
            </div>
          ) : (
            <div className="p-4 space-y-3">
              {inShopVehicles.map((v) => (
                <div key={v.id} className={`flex items-center gap-4 p-4 rounded-xl transition-colors ${isDark ? "bg-slate-800/50 hover:bg-violet-900/10" : "bg-slate-50 hover:bg-gradient-to-r hover:from-violet-50 hover:to-violet-100/40"}`}>
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${isDark ? "bg-amber-500/10" : "bg-gradient-to-br from-amber-50 to-amber-100/50"}`}>
                    <Wrench className="w-5 h-5 text-amber-500" />
                  </div>
                  <div className="flex-1 min-w-0">
                    ...
                    <p className={`text-[11px] ${isDark ? "text-slate-400" : "text-slate-400"}`}>
                      Plate: <span className={`font-mono font-medium ${isDark ? "text-slate-300" : "text-slate-600"}`}>{v.licensePlate}</span>
                    </p>
                  </div>
                  <div className="text-right shrink-0">
                    <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[11px] font-bold ${isDark ? "bg-amber-500/10 text-amber-400 border border-amber-500/20" : "bg-amber-50 text-amber-600"}`}>
        ...
        <div className={`${cardClass} p-5`}>
          <h3 className={`text-base font-bold mb-1 ${isDark ? "text-slate-100" : "text-slate-900"}`}>Compliance Breakdown</h3>
          <div className="flex items-baseline gap-2 mb-5">
            <span className={`text-3xl font-extrabold tabular-nums ${isDark ? "text-slate-100" : "text-slate-900"}`}>{compliancePct}%</span>
            <span className={`text-sm ${isDark ? "text-slate-500" : "text-slate-400"}`}>(Overall)</span>
          </div>

          <div className="grid grid-cols-3 gap-3 mb-5">
            {complianceCategories.map((cat) => (
              <div key={cat.label} className="text-center">
                <p className={`text-xs mb-1 ${isDark ? "text-slate-500" : "text-slate-500"}`}>{cat.label}</p>
                <p className={`text-lg font-extrabold ${isDark ? "text-slate-200" : "text-slate-900"}`}>{cat.pct}%</p>
              </div>
            ))}
          </div>
          ...
          <div className={`mt-5 p-4 rounded-xl border ${isDark ? "bg-violet-500/10 border-violet-500/20" : "bg-gradient-to-r from-violet-50 to-violet-100/30 border-violet-100"}`}>
            <div className="flex items-center justify-between mb-2">
              <span className={`text-xs font-medium ${isDark ? "text-violet-400" : "text-violet-600"}`}>Maint. Compliance</span>
              <span className={`text-xs font-bold ${isDark ? "text-violet-300" : "text-violet-700"}`}>{maintCompliancePct}%</span>
            </div>
            <div className={`h-2 rounded-full overflow-hidden ${isDark ? "bg-violet-900/30" : "bg-violet-200"}`}>
              <motion.div
                initial={{ width: 0 }}
