...
const card = "rounded-[14px] border transition-all duration-300 relative overflow-hidden backdrop-blur-xl shrink-0";
...
const darkCard = "bg-[#090D0B]/60 border-[#1E2B22]/50 shadow-[0_6px_20px_rgba(0,0,0,0.35)] hover:shadow-[0_8px_30px_rgb(0,0,0,0.4)]";
  ...
  return (
    <motion.div variants={stagger} initial="hidden" animate="visible" className="space-y-6">
      ...
      <motion.div variants={fadeIn} className="grid grid-cols-2 lg:grid-cols-4 gap-5">
        <div className={`group ${cardClass} p-5`}>
          ...
          <div className="flex items-center justify-between mb-3 relative z-10">
            <p className={`text-sm font-medium ${isDark ? "text-[#6B7C6B]" : "text-slate-500"}`}>Active Trips</p>
            <div className={`w-9 h-9 rounded-xl flex items-center justify-center transition-transform group-hover:scale-110 group-hover:shadow-[0_0_15px_rgba(34, 197, 94,0.3)] ${isDark ? "bg-emerald-500/10" : "bg-gradient-to-br from-emerald-50 to-emerald-100/50"}`}>
              <Route className="w-4 h-4 text-emerald-600" />
            </div>
          </div>
          <p className={`text-3xl font-extrabold tabular-nums relative z-10 ${isDark ? "text-[#E4E6DE]" : "text-slate-900"}`}>{kpi?.trips.active ?? 0}</p>
          <p className={`text-xs mt-2 relative z-10 ${isDark ? "text-[#4A5C4A]" : "text-slate-400"}`}>{kpi?.trips.pending ?? 0} drafts pending</p>
        </div>
        ...
        <div className={`group ${cardClass} p-5`}>
          ...
          {isDark && <div className="absolute top-0 right-0 w-32 h-32 bg-emerald-500/5 rounded-bl-full pointer-events-none transition-transform group-hover:scale-110" />}
          <div className="flex items-center justify-between mb-3 relative z-10">
            <p className={`text-sm font-medium ${isDark ? "text-[#6B7C6B]" : "text-slate-500"}`}>Fleet Available</p>
          ...
          <p className={`text-3xl font-extrabold tabular-nums relative z-10 ${isDark ? "text-[#E4E6DE]" : "text-slate-900"}`}>{kpi?.fleet.available ?? 0}</p>
          <p className={`text-xs mt-2 relative z-10 ${isDark ? "text-[#4A5C4A]" : "text-slate-400"}`}>{kpi?.fleet.onTrip ?? 0} on trip</p>
        </div>
        ...
        <div className={`group ${cardClass} p-5`}>
          ...
          <div className="flex items-center justify-between mb-3 relative z-10">
            <p className={`text-sm font-medium ${isDark ? "text-[#6B7C6B]" : "text-slate-500"}`}>Drivers On Duty</p>
          ...
          <p className={`text-3xl font-extrabold tabular-nums relative z-10 ${isDark ? "text-[#E4E6DE]" : "text-slate-900"}`}>{kpi?.drivers.onDuty ?? 0}</p>
          <p className={`text-xs mt-2 relative z-10 ${isDark ? "text-[#4A5C4A]" : "text-slate-400"}`}>of {kpi?.drivers.total ?? 0} total</p>
        </div>
        ...
        <div className={`group ${cardClass} p-5`}>
          ...
          <div className="flex items-center justify-between mb-3 relative z-10">
            <p className={`text-sm font-medium ${isDark ? "text-[#6B7C6B]" : "text-slate-500"}`}>Completed Today</p>
          ...
          <p className={`text-3xl font-extrabold tabular-nums relative z-10 ${isDark ? "text-[#E4E6DE]" : "text-slate-900"}`}>{kpi?.trips.completedToday ?? 0}</p>
      ...
      <motion.div variants={fadeIn} className={`${cardClass} p-0 overflow-hidden`}>
        {/* Header + filter tabs */}
        <div className={`px-6 py-4 flex items-center justify-between flex-wrap gap-3 border-b ${isDark ? "border-[#1E2B22]" : "border-slate-100"}`}>
          <h3 className={`text-base font-bold ${isDark ? "text-[#E4E6DE]" : "text-slate-900"}`}>Trips</h3>
              ...
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors ${
                  filter === s
                    ...
                    : isDark ? "bg-[#111A15] text-[#6B7C6B] hover:bg-[#1E2B22]" : "bg-slate-100 text-slate-600 hover:bg-slate-200"
                }`}
              >
        ...
        {filtered.length === 0 ? (
          <div className={`p-8 text-center text-sm ${isDark ? "text-[#4A5C4A]" : "text-slate-400"}`}>
          ...
          <div className="overflow-x-auto">
            <table className="min-w-[700px] w-full text-sm">
              <thead>
                <tr className={`border-b ${isDark ? "bg-[#090D0B] border-[#1E2B22]" : "bg-slate-50 border-slate-100"}`}>
              ...
              <tbody>
                {filtered.map((trip) => (
                  <tr key={trip.id} className={`border-b transition-colors ${isDark ? "border-[#1E2B22] hover:bg-[#182420]" : "border-slate-50 hover:bg-slate-50"}`}>
                    ...
                    <td className="px-6 py-3">
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className={`font-medium ${isDark ? "text-[#B0B8A8]" : "text-slate-900"}`}>{trip.origin}</span>
                        <span className={`text-${isDark ? "[#4A5C4A]" : "slate-300"}`}>→</span>
                        <span className={`font-medium ${isDark ? "text-[#B0B8A8]" : "text-slate-900"}`}>{trip.destination}</span>
                      </div>
                    </td>
                    <td className={`px-6 py-3 ${isDark ? "text-[#B0B8A8]" : "text-slate-600"}`}>{trip.driver?.fullName ?? <span className="text-slate-500">—</span>}</td>
                    <td className="px-6 py-3 font-mono text-xs text-slate-500">{trip.vehicle?.licensePlate ?? <span className="text-slate-500">—</span>}</td>
                    <td className={`px-6 py-3 text-xs ${isDark ? "text-[#6B7C6B]" : "text-slate-500"}`}>
                      {trip.scheduledDeparture
                        ? new Date(trip.scheduledDeparture).toLocaleDateString("en-IN", { day: "numeric", month: "short", hour: "2-digit", minute: "2-digit" })
                        : <span className="text-slate-500">—</span>}
                    </td>
                    <td className={`px-6 py-3 ${isDark ? "text-[#B0B8A8]" : "text-slate-600"}`}>{trip.distancePlanned ?? <span className="text-slate-500">—</span>}</td>
                    <td className="px-6 py-3">
                      <span className={`px-2.5 py-1 rounded-full text-xs font-semibold ${
                        isDark 
                          ? (trip.status === "COMPLETED" ? "bg-emerald-500/10 text-emerald-300/80" : trip.status === "DISPATCHED" ? "bg-emerald-500/10 text-emerald-300/80" : trip.status === "CANCELLED" ? "bg-red-500/10 text-red-300/80" : "bg-[#111A15] text-[#6B7C6B]")
      ...
      <motion.div variants={fadeIn} className="grid grid-cols-1 lg:grid-cols-3 gap-4 lg:gap-5">
        <div className={`${cardClass} p-5`}>
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className={`text-sm font-semibold ${isDark ? "text-[#E4E6DE]" : "text-slate-900"}`}>Fleet Status</h3>
          </div>
          <div className="space-y-3">
            {[
              { label: "Available", value: kpi?.fleet.available ?? 0, color: "bg-emerald-500" },
              { label: "On Trip", value: kpi?.fleet.onTrip ?? 0, color: "bg-emerald-500" },
              ...
              <div key={item.label} className="flex items-center gap-3">
                ...
                <span className={`text-sm flex-1 ${isDark ? "text-[#B0B8A8]" : "text-slate-600"}`}>{item.label}</span>
                <span className={`text-sm font-bold ${isDark ? "text-[#E4E6DE]" : "text-slate-900"}`}>{item.value}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className={`${cardClass} p-5`}>
          <div className="flex items-center gap-2 mb-4">
            <Users className="w-4 h-4 text-violet-600" />
            <h3 className={`text-sm font-semibold ${isDark ? "text-[#E4E6DE]" : "text-slate-900"}`}>Driver Status</h3>
          </div>
          <div className="space-y-3">
            {[
        ...
        <div className={`${cardClass} p-5`}>
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className={`text-sm font-semibold ${isDark ? "text-[#E4E6DE]" : "text-slate-900"}`}>Trip Breakdown</h3>
          </div>
          <div className="space-y-3">
            {(["DISPATCHED", "DRAFT", "COMPLETED", "CANCELLED"] as TripStatus[]).map((s) => (
              <div key={s} className="flex items-center gap-3">
                <span className={`w-2.5 h-2.5 rounded-full ${STATUS_DOT[s]}`} />
                <span className={`text-sm flex-1 capitalize ${isDark ? "text-[#B0B8A8]" : "text-slate-600"}`}>{s.charAt(0) + s.slice(1).toLowerCase()}</span>
                <span className={`text-sm font-bold ${isDark ? "text-[#E4E6DE]" : "text-slate-900"}`}>{counts[s]}</span>
              </div>
            ))}
          </div>
        </div>
