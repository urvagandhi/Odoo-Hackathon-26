...
const darkCard = "bg-[#090D0B]/60 border-slate-700/50 shadow-[0_6px_20px_rgba(0,0,0,0.35)] hover:shadow-[0_8px_30px_rgb(0,0,0,0.4)]";
  ...
  return (
    <motion.div variants={stagger} initial="hidden" animate="visible" className="space-y-6">
      ...
      <motion.div variants={fadeIn} className="grid grid-cols-2 lg:grid-cols-4 gap-5">
        <div className={`group ${cardClass} p-5`}>
          ...
          <div className="flex items-center justify-between mb-3 relative z-10">
            <p className={`text-sm font-medium ${isDark ? "text-[#6B7C6B]" : "text-slate-500"}`}>Total Fleet</p>
          ...
          <div className="flex gap-3 mt-2 text-xs relative z-10">
            ...
            <span className="text-emerald-600 font-medium">{kpi?.fleet.onTrip ?? 0} on trip</span>
          </div>
        </div>
        ...
        <div className={`group ${cardClass} p-5`}>
          ...
          <div className="flex items-center justify-between mb-3 relative z-10">
            <p className={`text-sm font-medium ${isDark ? "text-[#6B7C6B]" : "text-slate-500"}`}>Utilization</p>
          ...
          <div className={`mt-2 h-2 rounded-full overflow-hidden relative z-10 ${isDark ? "bg-[#111A15]" : "bg-slate-100"}`}>
            <motion.div
              initial={{ width: 0 }}
        ...
        <div className={`group ${cardClass} p-5`}>
          ...
          <p className={`text-xs mt-2 relative z-10 ${isDark ? "text-[#4A5C4A]" : "text-slate-400"}`}>of {kpi?.drivers.total ?? 0} total drivers</p>
        </div>
        ...
        <div className={`group ${cardClass} p-5`}>
          ...
          <p className={`text-xs mt-2 relative z-10 ${isDark ? "text-[#4A5C4A]" : "text-slate-400"}`}>{kpi?.trips.completedToday ?? 0} completed today</p>
        </div>
      </motion.div>
      ...
      <motion.div variants={fadeIn} className="grid grid-cols-1 lg:grid-cols-3 gap-4 lg:gap-5">
        ...
        <div className={`${cardClass} p-5`}>
          ...
          <div className="flex items-end justify-between mb-4">
            <div>
              <p className={`text-xs mb-1 ${isDark ? "text-[#4A5C4A]" : "text-slate-400"}`}>{currentMonth?.label ?? "Current month"}</p>
          ...
          <div className={`flex justify-between text-[10px] ${isDark ? "text-[#4A5C4A]" : "text-slate-400"}`}>
        ...
        <div className={`${cardClass} p-5`}>
          ...
          <div className="flex items-baseline gap-2 mb-3">
            ...
            <span className={`text-xs ${isDark ? "text-[#4A5C4A]" : "text-slate-400"}`}>this month</span>
          </div>
              ...
              {revenueSparkline.map((v, i) => {
                ...
                return (
                  <motion.div
                    ...
                    className={`flex-1 rounded-sm ${i === new Date().getMonth() ? "bg-violet-500 shadow-[0_0_10px_rgba(139,92,246,0.5)]" : (isDark ? "bg-[#111A15]" : "bg-slate-200")}`}
                  />
                );
              })}
            </div>
          )}
          <div className={`flex justify-between text-[10px] mt-2 ${isDark ? "text-[#4A5C4A]" : "text-slate-400"}`}>
            {["Jan","Mar","May","Jul","Sep","Nov"].map((m) => <span key={m}>{m}</span>)}
          </div>
        </div>
        ...
        <div className={`${cardClass} p-5`}>
          ...
          <div className="space-y-3">
            ...
            <div className={`flex items-center justify-between p-3 rounded-xl ${isDark ? "bg-[#111A15]" : "bg-gradient-to-r from-slate-50 to-slate-100/50"}`}>
              <div className="flex items-center gap-2">
                <Clock className="w-4 h-4 text-slate-500" />
                <span className={`text-sm ${isDark ? "text-[#B0B8A8]" : "text-slate-700"}`}>Pending Trips</span>
              </div>
              <span className={`font-bold ${isDark ? "text-[#B0B8A8]" : "text-slate-700"}`}>{kpi?.trips.pending ?? 0}</span>
            </div>
      ...
      <motion.div variants={fadeIn} className={`${cardClass} p-0 overflow-hidden`}>
        <div className={`px-6 py-4 border-b ${isDark ? "border-[#1E2B22]" : "border-slate-100"}`}>
          <h3 className={`text-base font-bold ${isDark ? "text-slate-100" : "text-slate-900"}`}>Recent Trips</h3>
        </div>
        {recentTrips.length === 0 ? (
          <div className={`p-8 text-center text-sm ${isDark ? "text-[#4A5C4A]" : "text-slate-400"}`}>No trips found. Add trips via Dispatch.</div>
        ) : (
            ...
            <tbody>
              {recentTrips.map((trip) => (
                <tr key={trip.id} className={`border-b transition-colors ${isDark ? "border-[#1E2B22] hover:bg-[#182420]" : "border-slate-50 hover:bg-slate-50"}`}>
                  ...
                  <td className={`px-6 py-3 ${isDark ? "text-[#B0B8A8]" : "text-slate-600"}`}>{trip.driver?.fullName ?? "—"}</td>
                  <td className="px-6 py-3 font-mono text-xs text-slate-500">{trip.vehicle?.licensePlate ?? "—"}</td>
                  <td className="px-6 py-3">
                    <span className={`px-2.5 py-1 rounded-full text-xs font-semibold ${
                      isDark 
                        ? (trip.status === "COMPLETED" ? "bg-[#14332A] text-[#86EFAC]" : trip.status === "DISPATCHED" ? "bg-[#162822] text-[#6EEAA0]" : "bg-[#1E2B22] text-[#B0B8A8]")
