  ...
  return (
    <header className={`h-16 border-b px-4 md:px-6 flex items-center gap-4 shrink-0 ${isDark ? 'bg-[#090D0B]/80 backdrop-blur-xl border-[#1A2620]' : 'bg-white border-slate-100'}`}>
      ...
      <div className="flex items-center gap-3">
        {/* Search */}
        <button className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all duration-200 ${isDark ? 'text-[#6B7C6B] hover:text-[#E4E6DE] hover:bg-[#162018]' : 'text-slate-400 hover:text-slate-600 hover:bg-slate-100'}`}>
          <Search className="w-[18px] h-[18px]" strokeWidth={1.8} />
        </button>

        {/* Notifications */}
        <button className={`relative w-10 h-10 rounded-xl flex items-center justify-center transition-all duration-200 ${isDark ? 'text-[#6B7C6B] hover:text-[#E4E6DE] hover:bg-[#162018]' : 'text-slate-400 hover:text-slate-600 hover:bg-slate-100'}`}>
        ...
        <div className="relative">
          ...
          >
            <img
              src={`https://ui-avatars.com/api/?name=${encodeURIComponent(user?.fullName ?? "User")}&background=1A2620&color=4ADE80&size=36&font-size=0.4&bold=true`}
              alt="Avatar"
