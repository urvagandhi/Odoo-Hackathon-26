...
/**
 * Retrieve vehicle image metadata for a given vehicle type name.
 *
 * Performs a case-insensitive exact lookup first, then a partial match where
 * either the provided name contains a known type key or a known type key
 * contains the provided name.
 *
 * @param typeName - The vehicle type name to match; may be partial, or `null`/`undefined`.
 * @returns The matching `VehicleImageInfo`, or `null` if `typeName` is falsy or no match is found.
 */
